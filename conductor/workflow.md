...
./db.sh start
./test_backend.sh
./db_migrate.sh revision "Your migration message"
```
...
graph TD
    subgraph "Standard Task Workflow"
        A[Start] --> B{Select Task};
        B --> C[Mark In Progress];
        C --> D{TDD Cycle};
        D --> E[Write Failing Tests];
        E --> F[Implement to Pass Tests];
        F --> G[Refactor Code];
        G --> H{Verify Coverage >80%};
        H --> I{Commit Changes};
        I --> J[Commit Code];
        J --> K[Attach Git Note Summary];
        K --> L[Record SHA in plan.md];
        L --> M[Commit plan.md];
        M --> N[End Task];
    end

    subgraph "Phase Completion Checkpointing Protocol"
        P[Phase Complete Trigger] --> Q[Announce Protocol];
        Q --> R{Ensure Phase Test Coverage};
        R --> S[Run Automated Tests];
        S --> T[Propose Manual Verification];
        T --> U{Await User Feedback};
        U --> V{Create Checkpoint};
        V --> W[Create Checkpoint Commit];
        W --> X[Attach Verification Report];
        X --> Y[Record Phase SHA in plan.md];
        Y --> Z[Commit plan.md];
        Z --> AA[End Phase];
    end
